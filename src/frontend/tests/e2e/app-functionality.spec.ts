...
test.describe('Personal Finance Planner E2E', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.getByRole('checkbox', { name: 'I understand that this' }).click();
    await page.getByRole('checkbox', { name: 'I understand that my data is' }).click();
  ...
  test('should navigate between all main pages', async ({ page }) => {
    ...
    // Test Cash Flows
    await page.getByRole('menuitem', { name: 'Navigate to Cash Flows' }).click();
    await expect(page.getByRole('heading', { name: 'Cash Flows' })).toBeVisible();
    
    // Test Projections
    await page.getByRole('menuitem', { name: 'Navigate to Projections' }).click();
    await expect(page.getByRole('heading', { name: 'Financial Projections' })).toBeVisible();
    
    // Test Scenarios
    await page.getByRole('menuitem', { name: 'Navigate to Scenarios' }).click();
    await expect(page.getByRole('heading', { name: 'Financial Scenarios' })).toBeVisible();
  ...
  test('should create and manage accounts', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Account Name' }).fill('Test Savings Account');
  ...
  test('should create and manage cash flows', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Description' }).fill('Test Monthly Income');
  ...
  test('should run financial projections', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Recalculate Projections' })).toBeVisible();
  ...
  test('should create and compare scenarios', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Scenario Name' }).fill('Test Scenario');
    await page.getByRole('button', { name: 'Create Scenario' }).click();
  ...
  test('should manage settings', async ({ page }) => {
    ...
    const autoSaveToggle = page.locator('text=Auto-save Data').locator('..').getByRole('checkbox');
  ...
  test('should show real financial calculations on dashboard', async ({ page }) => {
    await page.getByRole('menuitem', { name: 'Navigate to Dashboard' }).click();
